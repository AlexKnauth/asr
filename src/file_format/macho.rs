...
use crate::{signature::Signature, Address, PointerSize, Process};
    ...
    pub fn find_address(&self, symbol_name: &str) -> Option<Address> {
        ...
        #[cfg(feature = "std")]
        {
            ...
            for j in 0..self.number_of_symbols {
                ...
                if macho_bytes[string_end] == 0 && &macho_bytes[string_start..string_end] == symbol_name_bytes {
                    let symbol_fileoff: u64 = slice_read(macho_bytes, nlist_item + self.offsets.nlist_value).ok()?;
...
/// given.
fn slice_read<T: bytemuck::CheckedBitPattern, N: Into<u64>>(slice: &[u8], address: N) -> Result<T, bytemuck::checked::CheckedCastError> {
